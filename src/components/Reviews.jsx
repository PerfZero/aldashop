...
export default function Reviews({
  ...
  const openLightbox = (photos, index = 0) => {
    const imageUrls = photos.map(
      (photo) => `https://aldalinde.ru${photo.photo}`,
    );
    setImages(imageUrls);
    setPhotoIndex(index);
    setIsOpen(true);
  ...
  const fetchReviews = async () => {
    ...
    try {
      ...
      if (isAuthenticated && user) {
        const userReview = (data.results || []).find(
          (review) =>
  ...
  const handleSubmitReview = async () => {
    ...
    if (!modalRating || !modalTitle.trim() || !modalMessage.trim()) {
      ...
      setSubmitError(
        "Пожалуйста, поставьте оценку, укажите заголовок и напишите отзыв",
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          data.error || data.detail || "Ошибка при отправке отзыва",
        );
      }
    ...
    return [...Array(5)].map((_, index) => (
      ...
      >
        <path
          ...
          fill={index < numericRating ? "#844025" : "#fff"}
          stroke={index < numericRating ? "#844025" : "#844025"}
        />
      </svg>
    ));
  };
      ...
      <div className={styles.reviews}>
        ...
        <div className={styles.reviews__empty}>
          ...
          <p className={styles.reviews__empty_text}>
            Будьте первым, кто поделится мнением! Ваш отзыв поможет другим{" "}
            <br /> пользователям сделать правильный выбор.
          </p>
          {isAuthenticated ? (
            !userHasReviewed ? (
              <button
              ...
              <div className={styles.review__header}>
                <div className={styles.review__author}>
                  <div className={styles.review__author_info}>
                    ...
                    <span className={styles.review__location}>
                      {review.city}
                    </span>
                    <span className={styles.review__date}>
                      {new Date(review.date_create).toLocaleDateString(
                        "ru-RU",
                        {
                          day: "numeric",
                          month: "long",
                      ...
                      {review.photos.map((photo, index) => (
                        <div key={index} className={styles.review__image}>
                          {photo.photo ? (
                            ...
                            <div
                              className={styles.review__image_placeholder}
                            ></div>
                          )}
                        </div>
                      ))}
            ...
            >
              <div className={styles.modal_wrapper}>
                ...
                <p className={styles.modal__text}>
                  Пожалуйста, оставьте отзыв о нашем магазине и приобретенной
                  мебели. Ваш опыт поможет нам стать лучше и другим покупателям
                ...
                <div className={styles.modal__form}>
                  ...
                  <div className={styles.modal__upload_title}>
                    Добавьте фото
                  </div>
                  ...
                  >
                    <div className={styles.modal__upload_icon}>
                      ...
                      >
                        <path
                          d="M31.25 38L18.75 25.5L3.125 41.125V3.625H46.875V38M25 31.75L34.375 22.375L46.875 34.875V47.375H3.125V38"
                          stroke="#844025"
                          strokeWidth="1.5"
                          strokeLinecap="round"
                        ...
                        />
                        <path
                          d="M15.625 19.25C18.2138 19.25 20.3125 17.1513 20.3125 14.5625C20.3125 11.9737 18.2138 9.875 15.625 9.875C13.0362 9.875 10.9375 11.9737 10.9375 14.5625C10.9375 17.1513 13.0362 19.25 15.625 19.25Z"
                          stroke="#844025"
                          strokeWidth="1.5"
                          strokeLinecap="round"
                          strokeLinejoin="round"
                  ...
                  {submitError && (
                    <div className={styles.modal__error}>{submitError}</div>
                  )}
                  <button
                ...
                >
                  ...
                  >
                    <path
                      d="M0.5 0.5L15.5 15.5M15.5 0.5L0.5 15.5"
                      stroke="#C1AF86"
                      strokeLinecap="round"
                    />
                  </svg>
