  ...
  const openModal = async (type) => {
    ...
    try {
      const response = await fetch(
        `https://aldalinde.ru/api/documents?type=${apiType}`,
      );
      if (response.ok) {
        const data = await response.json();
        if (data.url) {
  ...
  return (
    <footer className={styles.footer}>
      <div className={styles.footerContent}>
        ...
        <div className={styles.footerSection}>
          ...
          <div className={styles.contactInfo}>
            <div>
              <p>
                <a
                  href={`tel:${(footerInfo?.phone || "9 87353 3435").replace(/\s/g, "")}`}
                >
            ...
            <p>
              Отдел продаж с{" "}
              {footerInfo?.sales_department_time_from?.substring(0, 5) ||
                "9:00"}{" "}
              до{" "}
              {footerInfo?.sales_department_time_to?.substring(0, 5) || "20:00"}
            </p>
          </div>
        </div>
        ...
        <div className={styles.footerSection}>
          ...
          <div className={styles.socialLinks}>
            {footerInfo?.youtube_link && (
              <a
                href={footerInfo.youtube_link}
                target="_blank"
                rel="noopener noreferrer"
              >
                <Image
                  src="/icon_footer_1.svg"
                  alt="YouTube"
                  width={24}
                  height={24}
                />
            ...
            {footerInfo?.instagram_link && (
              <a
                href={footerInfo.instagram_link}
                target="_blank"
                rel="noopener noreferrer"
              >
                <Image
                  src="/icon_footer_2.svg"
                  alt="Instagram"
                  width={24}
                  height={24}
            ...
            {footerInfo?.telegram_channel_link && (
              <a
                href={footerInfo.telegram_channel_link}
                target="_blank"
                rel="noopener noreferrer"
              >
                <Image
                  src="/icon_footer_3.svg"
      ...
      <div className={styles.footerBottom}>
        <div className={styles.copyright}>©2025 ALDA. Все права защищены</div>
