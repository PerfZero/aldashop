...
const pickMainPhoto = (photos = []) => photos.find((photo) => photo.main_photo) || photos[0];
const pickHoverPhoto = (photos = []) => photos.find((photo) => photo.photo_interior) || null;
...
const normalizeProduct = (source, fallbackModelId = null) => {
  ...
  const availableMaterials = source?.available_materials || productData.available_materials || [];
  const materialPhoto = source?.material_photo || productData.material_photo || null;
  const mainImage = toAbsoluteUrl(mainPhoto?.photo) || toAbsoluteUrl(materialPhoto?.photo_material) || FALLBACK_IMAGE;

  return {
    id: productData.id,
    modelId: source?.id || productData.model_id || fallbackModelId || productData.id,
    name: productData.title || source?.title || 'Товар',
    description:
      productData.short_description || source?.short_description || source?.description || 'Описание отсутствует',
...
export default function ProductCard({ product, filtersOpen = false, onProductClick }) {
  ...
  const [currentProduct, setCurrentProduct] = useState(() => normalizeProduct(product));
  const [selectedMaterialId, setSelectedMaterialId] = useState(
    product?.material_photo?.id || product?.available_materials?.[0]?.id || null,
  );
  ...
  useEffect(() => {
    ...
    setSelectedMaterialId(next.materialPhoto?.id || next.availableMaterials?.[0]?.id || null);
  }, [product]);
      ...
      const normalized = normalizeProduct(
        {
          ...
          ...data,
          product: data,
          available_materials: data.available_materials || currentProduct.availableMaterials,
  ...
  const mainPrice = hasDiscount ? currentProduct.discountedPrice : currentProduct.price;
  const currentMaterial =
    currentProduct.availableMaterials.find((item) => item.id === selectedMaterialId) || currentProduct.materialPhoto;

  return (
    <article className={styles.card}>
      {hasDiscount && <div className={styles.card__badge}>Sale</div>}
      {!hasDiscount && currentProduct.isBestseller && <div className={styles.card__badge}>Bestseller</div>}
        ...
        >
          {isMobile ? (
            <Swiper
              ref={swiperRef}
              modules={[Pagination]}
              pagination={currentProduct.hoverImage ? { clickable: true, dynamicBullets: false } : false}
              spaceBetween={0}
              slidesPerView={1}
      ...
      <div className={styles.card__content}>
        ...
        {currentProduct.description && <p className={styles.card__material_caption}>{currentProduct.description}</p>}
        ...
        {currentProduct.availableMaterials?.length > 0 ? (
          <>
                ...
                return (
                  <button
                    ...
                    className={`${styles.card__material} ${
                      selectedMaterialId === material.id ? styles.card__material_selected : ''
            ...
            {currentProduct.brand && <p className={styles.card__meta}>{currentProduct.brand}</p>}
            {!currentProduct.brand && currentMaterial?.title_material && (
              <p className={styles.card__meta}>{currentMaterial.title_material}</p>
            )}
          </>
        ) : (
          <>
            {currentProduct.strSizes && <p className={styles.card__meta}>{currentProduct.strSizes}</p>}
            {currentProduct.brand && <p className={styles.card__meta}>{currentProduct.brand}</p>}
          </>
        )}

        <div className={styles.card__price_row}>
          <p className={styles.card__price}>{mainPrice?.toLocaleString('ru-RU')} ₽</p>
          {hasDiscount && <p className={styles.card__price_old}>{currentProduct.price?.toLocaleString('ru-RU')} ₽</p>}
        </div>
      </div>
    </article>
  );
