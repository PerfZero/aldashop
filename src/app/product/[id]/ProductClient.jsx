...
export default function ProductClient({ initialProduct, productId }) {
  ...
  useEffect(() => {
    if (initialProduct) {
      const actualModelId = initialProduct.model_id || initialProduct.model?.id || initialProduct.id;
      ...
      if (initialProduct.sizes && initialProduct.available_sizes) {
        const matchingSize = initialProduct.available_sizes.find(s => s.id === initialProduct.sizes.id);
      ...
      <div className={styles.product}>
            ...
            <div className={styles.product__stars}>
              {[...Array(5)].map((_, index) => (
                <svg key={index} width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z" fill={index < (parseFloat(product.avg_rating) || 0) ? "#A45B38" : "#E5E5E5"} />
                </svg>
              ))}
            </div>
            <span className={styles.product__reviews} onClick={() => document.getElementById('reviews').scrollIntoView({ behavior: 'smooth' })} style={{ cursor: 'pointer' }}>
          ...
          {product.photos && product.photos.length > 0 && (
            <>
              <Swiper
                ...
                breakpoints={{
                  230: { slidesPerView: 4, direction: "horizontal" },
                  430: { slidesPerView: 4, direction: "horizontal" },
                  768: { slidesPerView: 6, direction: "vertical" },
                  1440: { slidesPerView: 5, direction: "vertical" },
                  1441: { slidesPerView: 7, direction: "vertical" }
                }}
                freeMode={true}
              ...
              <Swiper
                ...
                onSlideChange={(swiper) => setActiveThumbIndex(swiper.realIndex || swiper.activeIndex)}
                spaceBetween={10}
                navigation={true}
                watchSlidesProgress={true}
                observer={true}
        ...
        <div className={styles.product__info}>
          <div className={styles.product__header}>
            <h1 className={styles.product__title}>
              {product.title} 
              {product.bestseller && <div className={styles.product__bestseller}>Bestseller</div>}
              {hasDiscount && <div className={styles.product__bestseller}>Sale</div>}
              <button 
                className={`${styles.product__favorite_button} ${(isFavourite(product?.id) || product?.in_wishlist) ? styles.product__favorite_button_active : ''}`}
                onClick={handleToggleFavourite}
                aria-label={(isFavourite(product?.id) || product?.in_wishlist) ? 'Удалить из избранного' : 'Добавить в избранное'}
          ...
          <div id="rating" className={styles.product__rating}>
            <div className={styles.product__stars}>
              {[...Array(5)].map((_, index) => (
                <svg key={index} width="20" height="20" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z" fill={index < (parseFloat(product.avg_rating) || 0) ? "#A45B38" : "#E5E5E5"} />
                </svg>
              ))}
            </div>
          ...
          {product.param && product.param.length > 0 && (
            <div className={styles.product__params}>
              <div className={styles.product__section_header} onClick={() => setShowMaterialInfo(!showMaterialInfo)}>
          ...
          {product.photos && product.photos.some(photo => photo.photo_sizes === true) && (
            <div className={styles.product__actual_sizes}>
              <div className={styles.product__section_header} onClick={() => setShowActualSizes(!showActualSizes)}>
                ...
                <div className={styles.product__actual_sizes_photos}>
                  {product.photos.filter(photo => photo.photo_sizes === true).map((photo, index) => (
                    <div key={index} className={styles.product__actual_size_photo}>
                      <Image src={photo.photo} alt={`Фактические размеры - фото ${index + 1}`} width={300} height={300} unoptimized={true} />
                    </div>
                  ))}
                </div>
          ...
          {product.description && (
            <div className={styles.product__description}>
              <div className={styles.product__section_header} onClick={() => setShowProductInfo(!showProductInfo)}>
              ...
              <div className={`${styles.product__description_content} ${showProductInfo ? styles.product__content_visible : styles.product__content_hidden}`}>
                <p className={styles.product__description_paragraph}>{product.description}</p>
              </div>
            </div>
          )}
        </div>
